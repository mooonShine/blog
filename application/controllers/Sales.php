 ...
 * data: 16-12-6
 */
class Controller_Sales extends Base
{
    ...
    public function indexAction()
    {
        ...
        if (isset($totalPayment) && $totalPayment) {
            ...
            if ($newD < TIME_END_DAY && $newD > TIME_START_DAY) {
                $toList = $this->sales->getlist($where, '', '', true);
                $toList = $this->sales->total($toList['list'], $start_date);
        ...
        $list = $this->sales->getlist($where, $p);

        $list['list'] = $this->sales->total($list['list'], $start_date);
    ...
    public function addIncomeAction()
    {
        if ($this->_request->getPost()) {
            ...
            if (isset($data['type']) && $data['type'] == 'update') {
                $id = $data['id'];
                $count = $this->sales->count(array('id' => $id));
                if ($count) {
                    $pic = $this->sales->get('m_pic', array('id' => $id));
                    ...
                    if (isset($data['rebates']) && $data['rebates']) $info['rebates'] = $data['rebates'];
                    $info['update_time'] = time();
                    $re = $this->sales->update($info, array('id' => $id));
                ...
                $info['rebates'] = isset($data['rebates']) ? $data['rebates'] : '';
...
//                }
                $res = $this->sales->get('id', array('AND' => array('user_id' => $info['user_id'], 'date' => $info['date'], 'del' => 0)));
                if ($res) ajaxReturn(2, '每个客户每月只可添加一条数据');
                $re = $this->sales->insert($info);
                $content = $this->contentItem($contents);
    ...
    public function dataInfoAction()
    {
        if ($this->_request->getPost()) {
            ...
            if ($this->permi_id < PERMI_SALES) {
                $re = $this->sales->get('*', array('id' => $id));
            } else {
                $re = $this->sales->get(array('id', 'user_id', 'm_consume', 'refund', 'm_pic', 'rebates'), array('id' => $id));
            }
    ...
    public function delAction()
    {
        if ($this->_request->getPost()) {
            $id = $this->_request->getPost('id');
            if (!$id) ajaxReturn(0, '');
            $userinfo = $this->sales->get(array('id', 'user_id', 'b_money', 'm_consume', 'm_payment', 'total_payment', 'total_consume', 'refund', 'balance'), array('id' => $id));
            $user_name = $this->user->get('username', array('id' => $userinfo['user_id']));
            $re = $this->sales->update(array('del' => 1), array('id' => $id));
            if (!$re) ajaxReturn(0, '删除失败');
//            $m_pic = $this->sales->get('m_pic',array('id'=>$id));
//            $this->delPic($m_pic);
    ...
    public function updateStatusAction()
    {
        if ($this->_request->getPost()) {
            ...
            $statu = $status == 0 ? 1 : 0;
            $re = $this->sales->update(array('status' => $statu), array('id' => $id));

            if ($re) {
                $userid = $this->sales->get('user_id', array('id' => $id));
