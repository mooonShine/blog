...
class Controller_Agent extends Base{
    ...
    public function indexAction()
    {
        ...
        if(isset($totalPayment) && $totalPayment){
            ...
            if($newD<TIME_END_DAY && $newD>TIME_START_DAY){
                $toList = $this->agent->getlist($where,'','',true);
                $toList = $this->agent->total($toList['list'],$start_date);
        ...
        $list = $this->agent->getlist($where,$p);
        $list['list'] = $this->agent->total($list['list'],$start_date);
    ...
    public function addIncomeAction()
    {
        if($this->_request->getPost()){
            ...
            if(isset($data['type']) && $data['type']=='update') {
                $id = $data['id'];
                $count = $this->agent->count(array('id'=>$id));
                if($count){
                    $pic = $this->agent->get('m_pic',array('id'=>$id));
                    ...
                    $re = $this->agent->update($info,array('id'=>$id));
...
//                }
                $res = $this->agent->get('id',array('AND'=>array('user_id'=>$info['user_id'],'date'=>$info['date'],'del'=>0)));
                if($res)    ajaxReturn(2,'每个客户每月只可添加一条数据');
                $re = $this->agent->insert($info);
                $content=$this->contentItem($contents);
    ...
    public function dataInfoAction()
    {
        if($this->_request->getPost()){
            ...
            if($this->permi_id<PERMI_SALES){
                $re = $this->agent->get('*',array('id'=>$id));
            }else{
                $re = $this->agent->get(array('id','user_id','m_payment','refund','m_pic'),array('id'=>$id));
            }
    ...
    public function delAction()
    {
        if($this->_request->getPost()){
            $id = $this->_request->getPost('id');
            if(!$id) ajaxReturn(0,'');
            $userinfo=$this->agent->get(array('id','user_id','b_money','m_consume','m_payment','total_payment','total_consume','refund','balance'),array('id'=>$id));
            $user_name=$this->user->get('username',array('id'=>$userinfo['user_id']));
            $re = $this->agent->update(array('del'=>1),array('id'=>$id));
            if(!$re) ajaxReturn(0,'删除失败');
//            $m_pic = $this->agent->get('m_pic',array('id'=>$id));
//            $this->delPic($m_pic);
    ...
    public function updateStatusAction()
    {
        if($this->_request->getPost())
        {
            ...
            $statu=$status==0?1:0;
            $re= $this->agent->update(array('status'=>$statu),array('id'=>$id));

            if($re){
                $userid=$this->agent->get('user_id',array('id'=>$id));
