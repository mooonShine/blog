...
class Controller_Google extends Base{
    ...
    public function indexAction()
    {
        ...
        if(isset($totalPayment) && $totalPayment){
            ...
            if($newD<TIME_END_DAY && $newD>TIME_START_DAY){
                $toList = $this->google->getlist($where,'','',true);
                $toList = $this->google->total($toList['list'],$start_date);
        ...
        $list = $this->google->getlist($where,$p);
        $list['list'] = $this->google->total($list['list'],$start_date);
    ...
    public function addIncomeAction()
    {
        if($this->_request->getPost()){
            ...
            if(isset($data['type']) && $data['type']=='update') {
                $id = $data['id'];
                $count = $this->google->count(array('id'=>$id));
                if($count){
                    $pic = $this->google->get('m_pic',array('id'=>$id));
                    ...
                    if(isset($data['type_payment'])) $info['type'] = $data['type_payment'];
                    if(isset($data['dollar_m_consume']) && $data['dollar_m_consume']) $info['dollar_m_consume'] = $data['dollar_m_consume'];
                    $info['update_time'] =time();
                    $re = $this->google->update($info,array('id'=>$id));
                ...
                $info['dollar_m_consume'] = isset($data['dollar_m_consume']) ? $data['dollar_m_consume'] : '';
                $info['b_money'] = isset($data['b_money'])?$data['b_money']:'';
                $info['type'] = isset($data['type_payment'])&&$data['type_payment']?$data['type_payment']:0;
...
//                }
                $res = $this->google->get('id',array('AND'=>array('user_id'=>$info['user_id'],'date'=>$info['date'],'del'=>0)));
                if($res)    ajaxReturn(2,'每个客户每月只可添加一条数据');
                $re = $this->google->insert($info);
                $content=$this->contentItem($contents);
    ...
    public function dataInfoAction()
    {
        if($this->_request->getPost()){
            ...
            if($this->permi_id<PERMI_SALES){
                $re = $this->google->get('*',array('id'=>$id));
            }else{
                $re = $this->google->get(array('id','user_id','m_payment','refund','m_pic'),array('id'=>$id));
            }
    ...
    public function delAction()
    {
        if($this->_request->getPost()){
            $id = $this->_request->getPost('id');
            if(!$id) ajaxReturn(0,'');
            $userinfo=$this->google->get(array('id','user_id','b_money','m_consume','m_payment','total_payment','total_consume','refund','balance'),array('id'=>$id));
            $user_name=$this->user->get('username',array('id'=>$userinfo['user_id']));
            $re = $this->google->update(array('del'=>1),array('id'=>$id));
            if(!$re) ajaxReturn(0,'删除失败');
//            $m_pic = $this->google->get('m_pic',array('id'=>$id));
//            $this->delPic($m_pic);
    ...
    public function updateStatusAction()
    {
        if($this->_request->getPost())
        {
            ...
            $statu=$status==0?1:0;
            $re= $this->google->update(array('status'=>$statu),array('id'=>$id));

            if($re){
                $userid=$this->google->get('user_id',array('id'=>$id));
