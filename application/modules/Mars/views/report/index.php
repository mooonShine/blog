<nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> pc广告
...
<div class="page-container">
    <div class="text-c">
        <form action="/Mars/report/index" method="get">
            ...
            结束日：<input type="text" name="end_date" class="laydate-icon" id="end" value="<?php if(fn_get_val('end_date')){ echo fn_get_val('end_date');}else{ echo date('Y-m-d',time()-86400);} ?>" />
            <select name="status" id="status" class="select" style="width: 100px">
                <option value="">请选择</option>
                <option value="1" <?php if(fn_get_val("status")==1){ echo 'selected';}?>>广告名称</option>
                <option value="2" <?php if(fn_get_val("status")==2){ echo 'selected';}?>>广告ID</option>
                <option value="3" <?php if(fn_get_val("status")==3){ echo 'selected';}?>>推广楼盘</option>
                <option value="4" <?php if(fn_get_val("status")==4){ echo 'selected';}?>>用户名</option>
                <option value="5" <?php if(fn_get_val("status")==5){ echo 'selected';}?>>联系人</option>
                <option value="6" <?php if(fn_get_val("status")==6){ echo 'selected';}?>>手机号</option>
                <option value="7" <?php if(fn_get_val("status")==7){ echo 'selected';}?>>邮箱</option>
            </select>
    ...
    <div class="mt-5">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <tr class="text-c">
                ...
                <th width="120">手机号</th>
                <th width="150">邮箱</th>
                <th width="80">当日展现量</th>
                <th width="80">当日点击量</th>
                <th width="80">点击率</th>
            ...
            <?php if (isset($list)): foreach ($list as $value): ?>
                <tr class="text-c">
                    ...
                    <td class="text-c"><?php echo $value['date']?date("Y-m-d", $value['date']):'-'; ?></td>
    ...
    $(function () {
        $('.delBtn').click(function () {
            ...
            layer.confirm('是否确定重置密码？', {
                ...
                window.location.href = "/Mars/customer/reset/id/" + oid;
            });
        })
