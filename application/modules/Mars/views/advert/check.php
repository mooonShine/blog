...
<nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> 广告审核
...
<div class="page-container">
    <div class="text-c">
        <form action="/Mars/advert/check" method="get">
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <?php if (isset($list)): foreach ($list as $value):
                ...
                <tr class="text-c">
                    ...
                    <td class="text-c"><?php echo $status[$value['review_status']]; ?></td>
                    <td class="text-c">
                    ...
                    <td class="text-c">
                        <button class="btn btn-primary radius adopt" style="width: 60px" type="button" res="<?php echo $value['id']; ?>">通过</button>
                        <button class="btn btn-default radius checkAdvert" style="width: 60px" type="button" res="<?php echo $value['id']; ?>">不通过</button>
                    </td>
		        ...
		        <div class="leftPeo peoContent" id="leftPeo" >
		            <div class="peoTitle">人群包列表</div>
		                <div class="peoDetail">
		                    ...
		                    <ul>
                                <li>
                                    <div class="detailcontentL">无</div>
                                    <div class="detailcontentR"></div>
                                </li>
		                    </ul>
		                </div>
		            </div>
...
<div id="checkAdvert" style="display: none;">
    <div class="form_check ">
        <form action="/Mars/advert/edit" method="post" id="form2">
            <div class="form_div_check">
                <label class="form-label form_title ">审核不通过<span></span></label>
            </div>
            <div class="form_div_check">
                <label class="form-label col-xs-4 "><span class="c-red">&nbsp;</span>不通过原因：</label>
                <div class="col-xs-8 ">
                    <textarea  name="reason" rows="10" cols="30">
                    </textarea>
                </div>
            </div>
        </form>
...
<script>
    ...
    * */
    ...
    $(".adopt").click(function(){
        ...
        var data = 'id='+id+'&review_status=1';
        checkAdvert(data);
    })
    ...
    * */
     ...
     * 审核提交
     * */
    function  checkAdvert(data)
    {
        $.ajax({
            url: '/Mars/advert/checkAdvert',
            type: 'get',
            dataType: 'json',
            data: data,
            success: function (d) {
                if(d.ret==0){
                    layer.open({
                        content: '审核成功！',
                        yes: function(){ location.href='/advert/list';}
                    });
                }else{
                    layer.open({
                        content: d.msg,
    ...
    function picAdd(url,type)
    {
        if(url!=undefined){
            ...
            if(type==2){
                ...
                html +="<div  style='position: absolute;width: 50px; height: 50px' ></div> <EMBED src='"+url+"' quality=high bgcolor=#FFFFFF width='40' height='40' NAME='myMovieName' ALIGN='' TYPE='application/x-shockwave-flash' PLUGINSPAGE='http://www.macromedia.com/go/getflashplayer'> </EMBED> </object><a href='#'>更换图片</a>";
                $(".pic").children().remove();
                $(".pic").html(html);
            }
        }
