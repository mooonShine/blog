 ...
 * User: Administrator
 */
class Model_Google extends Smodel
{
    protected $table = "revenue_google";
    ...
    public function total($list,$start_date)
    {
        ...
        foreach($list as $j=>&$l){
            ...
            $dollar_total_consume = isset($listUser[$l['user_id']]['dollar_total_consume'])?$listUser[$l['user_id']]['dollar_total_consume']:0;
            $l['total_payment'] = number_format($total_payment+$l['m_payment'],2);
            $l['total_consume'] = number_format($total_consume+$l['m_consume'],2);
            $l['dollar_total_consume'] = number_format($dollar_total_consume+$l['dollar_m_consume'],2);
            if($l['type']){
                //余额=期初余额（￥/$）+本月应收（￥/$）-本月收款（￥/$）-退款
                $l['balance'] = number_format($l['b_money']+$l['m_payment']-$l['dollar_m_consume']-$l['refund'],2);
            }else{
                //余额=期初余额（￥/$）+本月应收（￥/$）-本月收款（￥/$）-退款
                $l['balance'] = number_format($l['b_money']+$l['m_payment']-$l['m_consume']-$l['refund'],2);
            }
        }
        return $list;
    }
